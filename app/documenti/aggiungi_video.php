...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tipologia = 3; // Video/Documentario
    ...
    if (empty($errors)) {
        // Prepara i valori specifici per i video
        $params = [
            'titolo' => $titolo,
        ...
        if (insertDocument($conn, $params)) {
            $message = "Video/documentario inserito con successo";
            $messageType = 'success';
            // Reindirizza dopo 2 secondi alla pagina index
            header("refresh:2;url=index.php");
        } else {
            $message = "Errore durante l'inserimento del video/documentario";
            $messageType = 'error';
        }
    } else {
...
<div class="container px-6 mx-auto grid">
    <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
        Aggiungi Nuovo Video/Documentario
    </h2>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <form action="aggiungi_video.php" method="POST" enctype="multipart/form-data">
                <!-- Input nascosto per la tipologia -->
                <input type="hidden" name="tipologia" value="3">
