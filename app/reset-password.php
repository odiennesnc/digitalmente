...
if (!empty($token)) {
    $stmt = $conn->prepare("SELECT id FROM utenti WHERE reset_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $validToken) {
    ...
    if (empty($password) || empty($confirm_password)) {
        ...
        $stmt = $conn->prepare("UPDATE utenti SET password = ?, reset_id = NULL WHERE id = ?");
        $stmt->bind_param("si", $hashed_password, $user_id);
        
        if ($stmt->execute()) {
            $message = 'La password è stata reimpostata con successo. Ora puoi accedere con la nuova password.';
            $messageType = 'success';
...
<head>
    ...
    <title>Reimposta password - Digitalmente</title>
                    ...
                    <div class="w-full">
                        ...
                        <form method="POST">
                            <label class="block text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Nuova password</span>
                            ...
                            <label class="block mt-4 text-sm">
                                ...
                                <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="***************" type="password" name="confirm_password" required>
                            </label>
