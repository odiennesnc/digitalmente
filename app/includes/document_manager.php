...
/**
 * Recupera tutti i documenti con i dettagli specifici per tipologia
 * 
 * @param mysqli $conn Connessione al database
 * @return array Array di documenti con i relativi dettagli
 */
function getAllDocumentsWithDetails($conn) {
    $documenti = [];
    
    // Query base per recuperare tutti i documenti
    $query = "SELECT db.*, a.argomento 
              FROM documenti_base db
              LEFT JOIN argomenti a ON db.argomenti_id = a.id 
              ORDER BY db.data_inserimento DESC";
    ...
    if ($result) {
        while ($documento = $result->fetch_assoc()) {
            ...
            // Recupera dettagli specifici in base alla tipologia
            switch($tipologia) {
                case 1: // Libro
                    $queryLibro = "SELECT * FROM documenti_libri WHERE documento_id = ?";
                    $stmt = $conn->prepare($queryLibro);
                    if ($stmt) {
                        $stmt->bind_param("i", $id);
                        $stmt->execute();
                        ...
                        $stmt->close();
                        if ($dettagli) {
                            $documento = array_merge($documento, $dettagli);
                        }
                    }
                    break;
                    
                case 2: // Rivista
                    $queryRivista = "SELECT * FROM documenti_riviste WHERE documento_id = ?";
                    $stmt = $conn->prepare($queryRivista);
                    if ($stmt) {
                        $stmt->bind_param("i", $id);
                        $stmt->execute();
                        $dettagli = $stmt->get_result()->fetch_assoc();
                ...
                case 3: // Video
                    $queryVideo = "SELECT * FROM documenti_video WHERE documento_id = ?";
                    $stmt = $conn->prepare($queryVideo);
                    if ($stmt) {
                        $stmt->bind_param("i", $id);
                        $stmt->execute();
                        $dettagli = $stmt->get_result()->fetch_assoc();
    ...
    try {
        ...
        switch ($tipologia) {
            case 1: // Libro
                ...
                $stmtLibro->bind_param("isssssss", $documento_id, $autore, $editore, $collana, $traduzione, $pagine, $indice, $bibliografia);
...
function getDocumentTypeName($typeId) {
    ...
    return isset($types[$typeId]) ? $types[$typeId] : 'Sconosciuto';
}
